...
\newcommand\autore{ \href{https://luciamarock.wixsite.com/home}{Luciano Ciamarone \& Dora Motèque}}
...
\section{Distributed Minds}

The \textit{Event Manager} does not simply call a sound in response to a gesture: it embodies a decision-making process. 
In the case of recurrent neural networks (RNNs), the \textit{Mind} of the canvas selects which sound to use according to an internal goal, integrating the history of past interactions and projecting them into future trajectories. 
In contrast, Markov Models operate through a continuous adjustment of transition probabilities, dynamically reshaping the space of possibilities and selecting sounds according to these evolving balances. 
In both approaches, the Event Manager is not a neutral interface but an active agent, an \textit{artificial mind} that learns and responds.

Customization plays an important role in this framework. 
Owners may eventually personalize their canvases, feeding additional \textit{vectorialized information} into the RNN-based model. 
Such information could encode preferences, contextual data, or even therapeutic settings, allowing each canvas to embody not only a general intelligence but also a unique, situated identity.

The Event Manager also functions as a hub that can forward or receive events across canvases. 
This networking capacity transforms individual works into \textit{social works}, synchronizing multiple installations into a distributed system. 
A gesture on one canvas can resonate across the globe, influencing the behavior of distant canvases connected through the internet. 
Here, gesture is no longer confined to a single body or object but expands into a shared, global experience.

Within this perspective, the \textit{Minds} are not merely interlocutors with the local user, but also mediators between distant users. 
They become nodes of communication, translating local actions into distributed resonances and weaving together collective interactions. 
The canvas, therefore, is both a site of intimate touch and a portal to a networked, global ecology of gestures and sounds.


\section{Licensing and Ownership}
\textit{Sounding Canvas} © 2024 by Luciano Ciamarone and Dora Motèque is licensed under the \href{https://creativecommons.org/licenses/by-nc-sa/4.0/}{Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License}.
\newline
...
This work has been entirely conceived, implemented, and authored by Luciano Ciamarone and Dora Motèque. \newline 
...
Past and Upcoming installations and exhibitions are listed in the "SoundingCanvas news page" listed at the end of the following links section. \newline
\newline
Links: 
\begin{itemize}
...
\begin{thebibliography}{References}
	\bibitem{ballet}
	Luciano Ciamarone \& Dora Motèque. 
	...
	\texttt{luciano.ciamarone@libero.it}, \texttt{doradynamicdesign@gmail.com}, \texttt{b.didonato@gold.ac.uk}.
